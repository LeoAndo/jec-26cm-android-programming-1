...
2. **翻訳PRを開いてから公開するまでは、`docs/` を触るPRをマージしません。** 訳したのとは別のモデルが訳文を原文と照合し（教員が読めない言語では、日本語へ訳し戻してから比べます）、検査と配布ZIPの確認を済ませて翻訳PRをマージします。照合の指摘は、全件が翻訳PRのコメントに残ります。日本語の変更が先に入った場合は、最新のmainで差分を訳し直します。
...
2026年9月16日〜24日に回答があったアンケートの結果です（2026年9月24日時点）。**回答数は10件（n=10）で、クラス全員を調べた結果ではありません。** 傾向を見るための標本として扱ってください。回答は匿名化して記録します。

| | Android Studio | Xcode | MacBook / OS |
| --- | --- | --- | --- |
| 回答1 | Panda 2 2025.3.2 | Xcode Version 26.3(17C529) | MacBook Air macOS Tahoe 26.3.1 |
| 回答2 | Android Studio Quail 4 | version 27 | macOS 27 |
| 回答3 | 2025.3.2 | 26.3 | 26.3.1 |
| 回答4 | 21.0.9+-14787801-b1163.94 aarch64 | 26.3 | 26.3.1 |
| 回答5 | Build #AI-253.30387.90.2532.14935130, built on February 25, 2026 | Version 26.3 (17C529) | 26.3.1（25D2128） |
| 回答6 | Android Studio Panda 2 \| 2025.3.2 | XcodeVersion 26.3 (17C529) | macOS Tahoe バージョン26.3.1 |
| 回答7 | 2026.1.4 | 26.3 | 26.6.2 |
| 回答8 | Android Studio Panda 2 \| 2025.3.2 | Version 26.3 (17C529) | macOS Tahoe バージョン26.5.2 |
| 回答9 | Android Studio Panda 2 \| 2025.3.2 …（以下略） | 26.3 | 26.5.2 |
| 回答10 | Android Studio Panda 2 \| 2025.3.2 …（以下略） | 26.3 | 26.6.2 |

- 表の内容は回答をそのまま記録したものです。製品名のない回答、バージョン番号のない回答、About Android Studioの画面から別の行を写した回答があり、表記はそろっていません。教員側で補完・推測はしていません。
- 回答9・10は、About Android Studioの画面の情報をまとめて貼り付けた回答です（それぞれ約4,700文字）。表には先頭の製品名とバージョンだけを載せ、残りは「…（以下略）」としました。省いた部分のBuild行とRuntime version行は、上の「開発環境：教員」ブロックと同じです。途中のmacOSの行は、それぞれのMacBookの回答（26.5.2・26.6.2）と同じです。
- windowsは利用していない（従来どおり、この点は変わりません）。
- Xcodeは本授業では使いませんが、学生の端末の状況を把握するためアンケート項目に含めました。

## 回答の読み取り（教員による推測。表には反映していません）

About Android Studioの画面は、macOSでは **Android Studio** メニュー → **About Android Studio** で開きます。上の「開発環境：教員」ブロックは、この画面の情報を写したものです。1行目が製品名とバージョン、2行目がBuild行、3行目がRuntime version行です。

- **回答3の `2025.3.2` は、Panda 2 2025.3.2のバージョン番号と思われます。** 製品名が省かれているだけで、教員環境と同じ版である可能性が高いと読めます。
- **回答4の `21.0.9+-14787801-b1163.94 aarch64` は、Android Studioのバージョンではないと思われます。** About Android Studioの `Runtime version` 行（同梱されているJetBrains Runtimeの版）を写した記入ミスと読めます。この文字列は、上の「開発環境：教員」ブロックの `Runtime version:` 行と完全に一致します。教員のPanda 2 2025.3.2と同じJetBrains Runtimeのビルドを積んでいるため、**回答4もPanda 2 2025.3.2である可能性が高い**と推測できます。ただし前後のバージョンのAndroid Studioが同じJetBrains Runtimeを同梱している可能性は否定できないため、**断定はできません。本人に聞き直す必要があります。**
- **回答5の `Build #AI-253.30387.90.2532.14935130, built on February 25, 2026` も、Android Studioのバージョンの行ではありません。** About Android StudioのBuild行を写したものと読めます。この文字列は、上の「開発環境：教員」ブロックの `Build` 行と完全に一致します。ビルド番号はリリースごとに違う番号が付く（`2532` の部分が2025.3.2に当たる）ため、別の版でも同じ値になりうる回答4より確度は高く、**回答5はPanda 2 2025.3.2と読めます。**
- **回答7の `2026.1.4` は、Quail 4のバージョン番号です。** Android Developersの公式ページで確かめました（[Android Studio Quail 4 Closed Issues](https://developer.android.com/studio/releases/fixed-bugs/studio/2026.1.4)、2026年9月24日確認）。製品名が省かれているだけで、回答2と同じ版と読めます。
- 上の4点はいずれも教員の読み取りであり、本人への確認は取れていません。表の記載は回答のまま据え置きます。
- 読み取りを含めて数えると、Panda 2 2025.3.2が10件中8件（回答1・3・4・5・6・8・9・10。回答4は未確定）、Quail 4が10件中2件（回答2・7）です。

## 教員環境とのバージョン差

- **回答2のQuail 4と、回答7の `2026.1.4`（Quail 4のバージョン番号）は、教員環境のPanda 2 | 2025.3.2より新しい版です。** 教材はPanda 2を前提に書かれており、各単元のSTEP 01で**Empty Views Activity**を選ぶ手順とPanda 2のウィザード画面のスクリーンショットを載せています。
- Quail 4で確かめたことと確かめていないことは、[共通資料：ちがうAndroid Studioのバージョンで進めるとき](docs/common/other-versions.html)の3-4・4-2にあります（2026年9月17日確認）。Quail 4で作ったひな形は、`MainActivity.java` と `activity_main.xml` がPanda 2と同じで、違いはビルドの設定だけでした。全11単元・77か所で、STEPごとのコードを入れてビルドを確かめ、Quail 4だから起きる失敗はありませんでした。**確かめていないのは、Quail 4からエミュレータでアプリを動かすことと、Android Studioの画面（ウィザードやメニュー）の見え方です。** 教科書のスクリーンショットと違って見える可能性は残ります。対応方針は教員が判断します。
- 2026年9月24日時点で、Android Studioの公式サイトの安定版は **Quail 4 | 2026.1.4 Patch 1** です（[リリースノート](https://developer.android.com/studio/releases)）。公式サイトからAndroid Studioを新しく入れると、Panda 2ではなくQuail 4が入ります。
- n=4からn=10になり、基準より新しい版の回答は4件中1件（回答2）から10件中2件（回答2・7）になりました。**割合は少し下がりましたが、件数は1件から2件に増えています。** 上の確かめていない点が残るので、リスクがなくなったわけではありません。また回答4のAndroid Studioの版は未確定なので、実際の内訳はこれと異なる可能性があります。
