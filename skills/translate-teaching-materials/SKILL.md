...
- 訳したのとは別のsubagentが、カタログの `source` と `translation` を並べて読み、意味の違い・訳し落とし・足しすぎ・操作順の入れ替わり・用語集との不一致を挙げる。教員側で読めない言語（`my`、`mn`、`zh-Hant-HK`）では、訳文を日本語へ訳し戻して原文と比べる。訳し戻しは、原文を見ずに行う（原文を見ながらだと、訳文が足りなくても原文に引きずられて補ってしまう）。照合の担当の決め方と記録の残し方は、下の「照合の担当と記録」に従う。
- 照合の記録がそろっていること。担当（どのモデルが訳し、どのモデルが照合したか）と照合範囲がPR本文に、指摘の全件がPRのコメントにある。照合のあとで変えた文も、照合し直してある。
...
2. **照合の担当に、資料を自分で読ませる。** このskill・用語集・日本語のページのパスと、照合する対訳を渡す。親が決まりを要約して渡すだけにしない。親の読み違いが、照合にもそのまま入るためである。確かめてほしい点を足してもよいが、照合はそれだけに限らない。訳し戻す言語では、最初は文のIDと訳文だけを渡し、原文と日本語のページは訳し戻しを終えてから渡す。原文を見ずに訳し戻すためである。
3. **照合の担当が挙げた指摘は、全件を翻訳PRのコメントに残す**（PRがなければ、照合のissueのコメントに残す）。「対応不要」も含め、親が要約せずに、照合の担当が書いたとおりに残す。オーナーが、指摘が落ちていないかを確かめられるようにするためである。PR本文のチェックだけでは、照合したかどうかが外から分からない（#254・#255・#256・#258 は、照合済みのチェックを付けたまま、照合せずにマージされた。PR #257 の本文）。ミャンマー語（`my`）の語は、コメントにも写さない（AGENTS.md §12）。ミャンマー語の照合では、照合の担当に、指摘をはじめから文のID（作業ファイルの `id`）と原文と訳し戻した日本語で書かせる。親があとから綴りを消すと、書いたとおりに残せないためである。
