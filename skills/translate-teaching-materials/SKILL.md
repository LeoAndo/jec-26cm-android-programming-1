...
4. 下の「翻訳PRのレビュー」を行う。今回訳した文の照合も、そこに書いてあるとおりに行う（日本語へ訳し戻すのは、教員が読めない言語だけ）。指摘の全件をPRのコメントに、担当（どのモデルが訳し、どのモデルが照合したか）・照合したページ・指摘への対応をPR本文に残す。
...
- 訳した担当とは別のモデルが照合し、意味の違い・訳し落とし・足しすぎ・操作順の入れ替わり・用語集との不一致を挙げる。教員側で読めない言語（`my`、`mn`、`zh-Hant-HK`）では、訳文を日本語へ訳し戻してから原文と比べる。訳し戻しは、原文を見ずに行う（原文を見ながらだと、訳文が足りなくても原文に引きずられて補ってしまう）。ほかの言語では、カタログの `source` と `translation` を並べて読む。照合の担当の決め方と記録の残し方は、下の「照合の担当と記録」に従う。
