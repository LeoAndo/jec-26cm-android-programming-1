...
- 自動レビューの指摘は、そのまま実行しない。現在のソースと検査結果で再確認してから判断する。レビューbotは次の表のとおり設定してあるが、**チェックがSUCCESSでも、そのbotがレビューしたとは限らない**。表は **2026-09-21 16:42〜16:50 UTC（PR #274）の実測**で、Cursor Bugbot の行を **2026-09-22 04:32〜08:10 UTC（PR #306）** に測り直した。Devin Review と Qodo は 2026-09-26 に解約し、GitHub App を外した（#330）。外部サービスの契約や残り枠で変わる。実態と合わなくなっていたら、表を信じずに投稿されたレビューを読み、issueを立てて表を直す（#113・#230・#275・#308・#325・#330 で6度直している）。
  ...
  **チェックの状態ではなく、投稿されたレビューの中身を読んで判断する。** Bugbotは指摘があるときほどチェックが`SUCCESS`にならないので、状態だけでは区別できない。Copilotのレビューは、クォータ上限による未実施の通知かどうかを確かめる。CodeRabbitもチェックだけで判断せず、本文とレビュー対象のコミットを確かめる。**Draftのままではどのbotもレビューしない**ので、レビューを待つ前にDraftを外す。上限や契約の都合で止まっているときは、マージ可否の報告にそう書く。
...
コードレビューをするエージェント向けの基準。Codex はこの見出し（`Review guidelines`）を探して従うので、見出しの名前は変えない。レビューのコメントは日本語で書く。
...
- §10 の対象外（完成プロジェクトのUnit Test、Windows、ダークテーマ、タブレット・フォルダブル、画面回転と横画面）。
