...
- **ミャンマー語の語を、issue・PR本文・レビューの返信に書かない。** GitHubは結合文字を落とすことがあり、**別の語に変わったまま保存される**。実測では、`ကြိုး` の並び（`U+1000` `U+103C` `U+102D` `U+102F` `U+1038`）が先頭の子音だけに減り、14コードポイントが10になった（#311・PR #317 で `gh api` と `curl` の両方で再現。コードブロックの中でも落ちる。同じリクエストで送ったほかのミャンマー語の語は無傷だったので、語によって起きる）。訳語は「27件の側」のように件数や行番号で指し、綴りはカタログと `i18n/my/glossary.md` に置く。コミットメッセージとカタログのファイルには、そのまま書いてよい。
