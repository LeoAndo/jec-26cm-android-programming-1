...
- 自動レビューの指摘は、そのまま実行しない。現在のソースと検査結果で再確認してから判断する。レビューbotは次の表のとおり設定してあるが、**チェックがSUCCESSでも、そのbotがレビューしたとは限らない**。表は **2026-09-21 16:42〜16:50 UTC（PR #274）の実測**で、Cursor Bugbot の行を **2026-09-22 04:32〜08:10 UTC（PR #306）** に測り直し、Codex と Copilot の行を **2026-09-26 04:53〜05:13 UTC（PR #331）** に測った。Devin Review と Qodo は 2026-09-26 に解約し、GitHub App を外した（#330）。外部サービスの契約や残り枠で変わる。実態と合わなくなっていたら、表を信じずに投稿されたレビューを読み、issueを立てて表を直す（#113・#230・#275・#308・#325・#330 で6度直している）。
  ...
  **チェックの状態ではなく、投稿されたレビューの中身を読んで判断する。** Bugbotは指摘があるときほどチェックが`SUCCESS`にならないので、状態だけでは区別できない。**Codexはチェックを出さない**ので、レビューか、Codex（`chatgpt-codex-connector[bot]`）の👍のリアクションが付いたかを見る。解除やpushから5分ほどたってもどちらも付かなければ、Codexは動いていない。リアクションを付けたbotは `gh api repos/{owner}/{repo}/issues/<番号>/reactions` で確かめる（Bugbot（`cursor[bot]`）も、動いているあいだだけ👀を付ける）。Copilotは投稿しなくなったが、投稿があれば、クォータ上限による未実施の通知かどうかを確かめる。CodeRabbitもチェックだけで判断せず、本文とレビュー対象のコミットを確かめる。**Draftのままではどのbotもレビューしない**ので、レビューを待つ前にDraftを外す。上限や契約の都合で止まっているときは、マージ可否の報告にそう書く。
