...
- 自動レビューの指摘は、そのまま実行しない。現在のソースと検査結果で再確認してから判断する。4つのbotが設定してあるが、**チェックがSUCCESSでも、そのbotがレビューしたとは限らない**。次の表は **2026-09-21 16:42〜16:50 UTC（PR #274）の実測**で、Cursor Bugbot の行を **2026-09-22 04:32〜08:10 UTC（PR #306）**、Devin Review の行を **2026-09-22 08:08〜08:41 UTC（PR #305・#307・#309）** と **2026-09-24 13:50 UTC（PR #324）** に測り直した。外部サービスの契約や残り枠で変わる。実態と合わなくなっていたら、表を信じずに投稿されたレビューを読み、issueを立てて表を直す（#113・#230・#275・#308・#325 で5度直している）。
  ...
  | Devin Review | 何も投稿しない | 契約や残り枠しだいで、レビューするときとしないときがある。**するとき**は、解除の1〜2分後にレビューを1件投稿する。指摘がなければ `✅ Devin Review: No Issues Found`、あれば `Devin Review found N potential issue.` とインライン指摘（実測：#305 は59秒で指摘なし、#307 は1分40秒でインライン1件、#309 は1分8秒で指摘なし）。**しないとき**は、レビューを投稿せず、commit status（context：`Devin Review`）の説明が `Full review skipped: trial expired and no credits remaining` になる（#274・#324。`gh pr checks` では説明の欄に出る）。**レビューしてもしなくても、指摘があってもなくても、チェックは`pass`** | レビューするときは出る |
  | Copilot | — | レビューを1件投稿するが、内容は「クォータ上限のためレビューできない」 | 出ない |

  **チェックの状態ではなく、投稿されたレビューの中身を読んで判断する。** Bugbotは指摘があるときほどチェックが`SUCCESS`にならず、**Devin Reviewは指摘があってもなくてもチェックが`pass`になる**ので、状態だけでは区別できない。Copilotのレビューは、クォータ上限による未実施の通知かどうかを確かめる。CodeRabbitもチェックだけで判断せず、本文とレビュー対象のコミットを確かめる。**Draftのままではどのbotもレビューしない**ので、レビューを待つ前にDraftを外す。上限やトライアル終了で止まっているときは、マージ可否の報告にそう書く。
