...
- 自動レビューの指摘は、そのまま実行しない。現在のソースと検査結果で再確認してから判断する。4つのbotが設定してあるが、**チェックがSUCCESSでも、そのbotがレビューしたとは限らない**。次の表は **2026-09-21 16:42〜16:50 UTC（PR #274）の実測**で、Cursor Bugbot の行を **2026-09-22 04:32〜08:10 UTC（PR #306）**、Devin Review の行を **2026-09-22 08:08〜08:41 UTC（PR #305・#307・#309）** に測り直した。外部サービスの契約や残り枠で変わる。実態と合わなくなっていたら、表を信じずに投稿されたレビューを読み、issueを立てて表を直す（#113・#230・#275・#308 で4度直している）。
