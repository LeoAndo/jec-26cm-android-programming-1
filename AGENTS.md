...
- **教科書のHTMLは、開始タグと終了タグを必ず対応させる。** `<p>`・`<li>`・`<td>` の閉じ忘れ、`<span/>` のような自己終了タグがあると、文を取り出せない。`python3 scripts/localize-student-materials.py check` が行番号つきで落ちる（§7の `unittest` にも含まれる）。
