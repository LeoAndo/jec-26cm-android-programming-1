...
- PR本文は「概要／変更内容／判断したこと／検証／実績」の順に書き、`Closes #<番号>` を入れる。検証の節に「※ リポジトリの方針により、Unit Test は対象外です。」と書く。
  ...
  - PR本文の仕上げは、**Draftを解除したあと、Cursor Bugbotのチェックが終わってから**にする。Bugbotが動く契機はpushではなくDraftの解除で、Draftのあいだはチェックそのものが現れない（PR #274 の実測）。解除の直後に `pending` になり、2分前後で終わる。Bugbotはレビュー開始時点の本文に自分の要約（`<!-- CURSOR_SUMMARY -->`）を足して本文ごと書き戻すので、この2分前後に `gh pr edit --body` すると、コマンドは成功したように見えて本文が黙ってひな形へ巻き戻る（PR #267 で検証や照合の記録が消えた）。ひな形でDraft PRを作らず、完成版の本文で `gh pr create` するのが確実。本文を書き換えたあとは `gh pr view <番号> --json body` で読み直し、書いた内容が残っていることを確かめる。
