...
- PR本文は「概要／変更内容／判断したこと／検証／実績」の順に書き、`Closes #<番号>` を入れる。検証の節に「※ リポジトリの方針により、Unit Test は対象外です。」と書く。
  ...
  - PR本文の仕上げは、**Cursor Bugbotのチェックが終わってから**にする。Bugbotはレビュー開始時点の本文に自分の要約（`<!-- CURSOR_SUMMARY -->`）を足して本文ごと書き戻すので、レビュー中（pushから2分前後）に `gh pr edit --body` すると、コマンドは成功したように見えて本文が黙ってひな形へ巻き戻る（PR #267 で検証や照合の記録が消えた）。ひな形でDraft PRを作らず、完成版の本文で `gh pr create` するのが確実。本文を書き換えたあとは `gh pr view <番号> --json body` で読み直し、書いた内容が残っていることを確かめる。
...
- 自動レビューの指摘は、そのまま実行しない。現在のソースと検査結果で再確認してから判断する。4つのbotが動くが、**チェックがSUCCESSでも、そのbotがレビューしたとは限らない**（2026-09-21時点）。
  ...
  | Devin Review | 指摘の有無にかかわらずチェックは`SUCCESS`。指摘があればインラインに投稿する | 出る |
  | Copilot | レビューを1件投稿するが、内容は「クォータ上限のためレビューできない」 | 出ない |

  **チェックの状態ではなく、投稿されたレビューの中身を読んで判断する。** Bugbotは指摘があるときほどチェックが`SUCCESS`にならず、Devin Reviewは`SUCCESS`でも指摘がある。Copilotのレビューは、クォータ上限による未実施の通知かどうかを確かめる。CodeRabbitもチェックだけで判断せず、本文とレビュー対象のコミットを確かめる。上限で止まっているときは、マージ可否の報告にそう書く。
