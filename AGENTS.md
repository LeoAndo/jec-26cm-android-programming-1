...
- 自動レビューの指摘は、そのまま実行しない。現在のソースと検査結果で再確認してから判断する。レビューbotは次の表のとおり設定してあるが、**チェックがSUCCESSでも、そのbotがレビューしたとは限らない**。表は **2026-09-21 16:42〜16:50 UTC（PR #274）の実測**で、Cursor Bugbot の行を **2026-09-22 04:32〜08:10 UTC（PR #306）** に測り直し、Codex と Copilot の行を **2026-09-26 04:53〜05:07 UTC（PR #331）** に測った。Devin Review と Qodo は 2026-09-26 に解約し、GitHub App を外した（#330）。外部サービスの契約や残り枠で変わる。実態と合わなくなっていたら、表を信じずに投稿されたレビューを読み、issueを立てて表を直す（#113・#230・#275・#308・#325・#330 で6度直している）。
  ...
  | Codex | 何も投稿しない（PR #331 で約6分待ち、レビュー・コメント・リアクション・チェックのどれもなかった） | 解除の2分半ほどあとに、`chatgpt-codex-connector[bot]` がレビューを1件投稿する（PR #331 は2分36秒）。指摘はインラインで、先頭に `P2` などの優先度のバッジが付き、根拠にした行が `AGENTS.md reference` として添えられる。**チェックも commit status も出さない。** Codex自身の説明では、レビューするのはPRを開いたとき・Draftを外したとき・`@codex review` とコメントしたときだけで、指摘がなければレビューの代わりにPRへ👍のリアクションを付ける | 出る。規則は末尾の「Code Review Rules」 |
  | Copilot | 何も投稿しない | 何も投稿しない（PR #326・#329・#331）。PR #324（2026-09-24 13:50 UTC）までは、「クォータ上限のためレビューできない」というレビューを1件投稿していた | 出ない |

  **チェックの状態ではなく、投稿されたレビューの中身を読んで判断する。** Bugbotは指摘があるときほどチェックが`SUCCESS`にならないので、状態だけでは区別できない。**Codexはチェックを出さない**ので、レビューか👍のリアクションが付いたかを見る。解除から5分ほどたってもどちらも付かなければ、Codexは動いていない。pushしても見直さないので、直したあとに見てほしいときは `@codex review` とコメントする。Copilotは投稿しなくなったが、投稿があれば、クォータ上限による未実施の通知かどうかを確かめる。CodeRabbitもチェックだけで判断せず、本文とレビュー対象のコミットを確かめる。**Draftのままではどのbotもレビューしない**ので、レビューを待つ前にDraftを外す。上限や契約の都合で止まっているときは、マージ可否の報告にそう書く。
